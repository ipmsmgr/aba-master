...
module.exports = function (lang) {
  ...
  function assembleTraits(personalityTree) {
    ...
    switch (relevantBig5.length) {
      ...
      case 5:
        ...
        sentences.push(format(tphrase("Subject is %s, %s and %s"), adj1, adj2, adj3) + ".");
        break;
    }
  ...
  function assembleFacets(personalityTree) {
    ...
    sentences.push(format(tphrase("Subject is %s"), info.term) + ": " + info.description + ".");
    ...
    sentences.push(format(tphrase("And subject is %s"), info.term) + ": " + info.description + ".");
  ...
  function assembleValues(valuesTree) {
    ...
    if (sameQI) {
      ...
      switch (intervalFor(valuesList[0].percentage)) {
        case 0:
          sentence = format(tphrase("Subject is relatively unconcerned with both %s and %s"), term1, term2) + ".";
          break;
        case 1:
          sentence = format(tphrase("Subject doesn't find either %s or %s to be particularly motivating for them"), term1, term2) + ".";
          break;
        case 2:
          sentence = format(tphrase("Subject values both %s and %s a bit"), term1, term2) + ".";
          break;
        case 3:
          sentence = format(tphrase("Subject considers both %s and %s to guide a large part of what they do"), term1, term2) + ".";
          break;
      }
      ...
      for (i = 0; i < valuesInfo.length; i += 1) {
        ...
        switch (intervalFor(valuesList[i].percentage)) {
          case 0:
            sentence = format(tphrase("Subject is relatively unconcerned with %s"), valuesInfo[i].term);
            break;
          case 1:
            sentence = format(tphrase("Subject doesn't find %s to be particularly motivating for them"), valuesInfo[i].term);
            break;
          case 2:
            sentence = format(tphrase("Subject values %s a bit more"), valuesInfo[i].term);
            break;
          case 3:
            sentence = format(tphrase("Subject considers %s to guide a large part of what they do"), valuesInfo[i].term);
            break;
        }
  ...
  function assembleNeeds(needsTree) {
    ...
    switch (intervalFor(needsList[0].percentage)) {
      case 0:
        sentence = tphrase("Experiences that make subject feel high %s are generally unappealing to them");
        break;
      case 1:
        sentence = tphrase("Experiences that give a sense of %s hold some appeal to them");
        break;
      case 2:
        sentence = tphrase("Subject is motivated to seek out experiences that provide a strong feeling of %s");
        break;
      case 3:
        sentence = tphrase("Subject's choices are driven by a desire for %s");
        break;
    }
