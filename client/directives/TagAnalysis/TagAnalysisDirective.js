...
angular.module('pi').directive('piTagAnalysis', ['DataService', 'ApiService', 'MetricsService', 'ChartDataService', function (DataService, ApiService, MetricsService,ChartDataService) {
    ...
    function linkFunction(scope, iElement, iAttrs, ctrl) {
        ...
        scope.$on('visualizeTags', function (event, args) {
            convertToChartData(args.scores, args.analyses);
        });
        ...
        var convertToChartData = function (scores, subjectAnalyses) {
            ...
            //var all = ChartDataService.getChartData(subjectAnalyses,scores);
            
            //console.log(JSON.stringify(DataService.extractKeysFromAnalyses([subjectAnalyses[0]])));
            ...
            for (var i = 0; i < subjectAnalyses.length; i++) {
                ...
                var analysisScores = ChartDataService.flattenScores(analysis);
                analysis.scores = {};
            ...
            for (var i = 0; i < all.length; i++) {
                ...
                for (var key in all[i].scores) {
                    ...
                    // Identify the label for this metric. The key
                    // is the ID but we want the name of it.
                    var label = key;
                    var metric = MetricsService.getMetricById(key);
                    if(metric!=null){
                        label = metric.name;
                    }
    ...
    return {
        ...
        templateUrl: 'directives/TagAnalysis/TagAnalysisTemplate.html',
        link: linkFunction
    }
            }]);
